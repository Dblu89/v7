...
ind = {
    ...
    "volume": v, "open_next": np.concatenate([o[1:],[c[-1]]]),
}

for p in [3,5,8,9,10,13,20,21,34,50,100,200]:
    a=2/(p+1); out=np.empty_like(c); out[0]=c[0]
    for i in range(1,n): out[i]=a*c[i]+(1-a)*out[i-1]
    ind[f"ema_{p}"]=out
...
for fast,slow in [(3,10),(5,13),(8,21),(12,26)]:
    af=2/(fast+1); as_=2/(slow+1)
    ef=np.empty_like(c); ef[0]=c[0]
    es=np.empty_like(c); es[0]=c[0]
    for i in range(1,n):
        ef[i]=af*c[i]+(1-af)*ef[i-1]
        es[i]=as_*c[i]+(1-as_)*es[i-1]
    mac=ef-es; sig=np.empty_like(c); sig[0]=mac[0]; a9=2/10
    for i in range(1,n): sig[i]=a9*mac[i]+(1-a9)*sig[i-1]
    ind[f"macd_{fast}_{slow}_hist"]=mac-sig

for p in [5,10,20,50]:
    s=pd.Series(c); sma=s.rolling(p).mean().values; std=s.rolling(p).std().values
    for mf,tag in [(1.0,"10"),(1.5,"15"),(2.0,"20"),(2.5,"25"),(3.0,"30")]:
        up=sma+mf*std; lo=sma-mf*std
        ind[f"bb_{p}_{tag}_pct"]=(c-lo)/(up-lo+1e-9)
        ind[f"bb_{p}_{tag}_upper"]=up; ind[f"bb_{p}_{tag}_lower"]=lo
...
for p in [7,10,14,20,30]:
    tp=(h+l+c)/3; sma=pd.Series(tp).rolling(p).mean().values
    mad=pd.Series(tp).rolling(p).apply(lambda x: np.abs(x-x.mean()).mean()).values
    ind[f"cci_{p}"]=(tp-sma)/(0.015*mad+1e-9)

# VWAP diario
vwap_arr=np.full(n,np.nan); tp=(h+l+c)/3
cum_tpv=np.zeros(n); cum_vol=np.zeros(n)
datas=df.index.date; data_atual=None
for i in range(n):
    if datas[i]!=data_atual:
        data_atual=datas[i]; cum_tpv[i]=tp[i]*v[i]; cum_vol[i]=v[i]
    else:
        cum_tpv[i]=cum_tpv[i-1]+tp[i]*v[i]; cum_vol[i]=cum_vol[i-1]+v[i]
    if cum_vol[i]>0: vwap_arr[i]=cum_tpv[i]/cum_vol[i]
ind["vwap"]=vwap_arr

vwap_std=np.full(n,np.nan); sq_sum=np.zeros(n); cnt=np.zeros(n); data_atual=None
for i in range(n):
    if datas[i]!=data_atual:
        data_atual=datas[i]; sq_sum[i]=(c[i]-vwap_arr[i])**2; cnt[i]=1
    else:
        sq_sum[i]=sq_sum[i-1]+(c[i]-vwap_arr[i])**2; cnt[i]=cnt[i-1]+1
    if cnt[i]>1: vwap_std[i]=np.sqrt(sq_sum[i]/cnt[i])
ind["vwap_std"]=vwap_std
ind["vwap_upper1"]=vwap_arr+vwap_std; ind["vwap_lower1"]=vwap_arr-vwap_std
ind["vwap_upper2"]=vwap_arr+2*vwap_std; ind["vwap_lower2"]=vwap_arr-2*vwap_std

for p in [10,20]:
    vm=pd.Series(v).rolling(p).mean().values; vs=pd.Series(v).rolling(p).std().values
    ind[f"vol_z_{p}"]=(v-vm)/(vs+1e-9); ind[f"vol_ratio_{p}"]=v/(vm+1e-9)
...
for orb_min in [5,10,15,20,30,45]:
    ...
    day_data={}
    for i in range(n):
        dt=df.index[i]; data=dt.date(); mins=dt.hour*60+dt.minute-9*60
        if mins<0: continue
        ...
        if not day_data[data]["done"]:
            ...
            else: day_data[data]["done"]=True
        if day_data[data]["done"] or mins>orb_min:
            orb_high[i]=day_data[data]["hi"]; orb_low[i]=day_data[data]["lo"]
    ind[f"orb_high_{orb_min}"]=orb_high; ind[f"orb_low_{orb_min}"]=orb_low

hora=df.index.hour
ind["session_am"]=((hora>=9)&(hora<12)).astype(np.int8)
ind["session_pm"]=((hora>=13)&(hora<17)).astype(np.int8)
ind["hora"]=hora; ind["dow"]=df.index.dayofweek.values
return ind
```
...
n=len(on); pnls=np.empty(n,dtype=np.float64)
n_tr=0; em=False; ep=sl=tp=0.0
for i in range(n-1):
if em:
if lo[i]<=sl or hi[i]>=tp or ext[i]:
saida=sl if lo[i]<=sl else (tp if hi[i]>=tp else on[i])
pnls[n_tr]=(saida-ep)*mult-comm-slip*mult*0.1; n_tr+=1; em=False
continue
if ent[i] and not em:
ep=on[i]
if np.isnan(ep) or ep<=0: continue
sl=ep-sl_pts; tp=ep+tp_pts; em=True
return pnls[:n_tr]
...
pnls[n_tr]=(ep-saida)*mult-comm-slip*mult*0.1; n_tr+=1; em=False
continue
if ent[i] and not em:
ep=on[i]
if np.isnan(ep) or ep<=0: continue
...
n_tr=0; em=False; ep=sl=tp=0.0; entry_idx=0
...
for i in range(n-1):
    ...
    if ent[i] and not em:
        ep=on[i]
        if np.isnan(ep) or ep<=0: continue
        sl=ep+sl_pts; tp=ep-tp_pts; em=True; entry_idx=i

return (pnls[:n_tr], ctx_atr[:n_tr], ctx_vol[:n_tr],
        ctx_rsi[:n_tr], ctx_dvwap[:n_tr],
        ctx_hora[:n_tr], ctx_dow[:n_tr])
```

@njit(cache=True)
def calcular_mae_mfe(on,hi,lo,ent,direction,max_bars=60):
n=len(on); mae=np.empty(n,dtype=np.float64); mfe=np.empty(n,dtype=np.float64); n_tr=0
for i in range(n-max_bars):
if not ent[i]: continue
ep=on[i]
if np.isnan(ep) or ep<=0: continue
mf=mc=0.0
...
if fav>mf: mf=fav
if con>mc: mc=con
mae[n_tr]=mc; mfe[n_tr]=mf; n_tr+=1
return mae[:n_tr],mfe[:n_tr]

def executar(ind,ent,ext,sl_pts,tp_pts,direction):
on=ind[“open_next”].astype(np.float64)
hi=ind[“high”].astype(np.float64); lo=ind[“low”].astype(np.float64)
e=ent.astype(np.bool_); x=ext.astype(np.bool_)
if direction==“long”:
...
if len(l)==0 or len(w)==0: return None
pf=abs(w.sum()/l.sum())
if pf>MAX_PF: return None
eq=np.concatenate([[CAPITAL],CAPITAL+np.cumsum(pnls)])
pk=np.maximum.accumulate(eq); mdd=float(((eq-pk)/pk*100).min())
if mdd<MAX_DD: return None
ret=pnls/CAPITAL; sh=float(ret.mean()/(ret.std()+1e-9)*np.sqrt(252*390))
if sh>MAX_SHARPE: return None
jan_pos=sum(1 for s in range(0,max(1,len(pnls)-30),15) if pnls[s:s+30].sum()>0)
n_jan=max(1,len(range(0,max(1,len(pnls)-30),15)))
return {“n”:len(pnls),“wr”:round(len(w)/len(pnls)*100,2),
“pf”:round(pf,3),“sh”:round(sh,3),
“exp”:round(float(pnls.mean()),2),
“pnl”:round(float(pnls.sum()),2),
“mdd”:round(mdd,2),“jan_pos”:round(jan_pos/n_jan*100,1)}

def score(m):
if not m: return 0
return (min(m[“pf”],MAX_PF)/MAX_PF*0.30+
...
if len(pnls) < 50: return None

winners = pnls > 0
losers  = pnls <= 0
n_win   = winners.sum(); n_los = losers.sum()

if n_win < 10 or n_los < 10: return None

# Analisar cada feature nos losers vs winners
features = {
    "atr":      ctx_atr,
    "vol":      ctx_vol,
    "rsi":      ctx_rsi,
...
for feat_nome, feat_vals in features.items():
    # Mediana dos losers vs winners
    med_los = np.median(feat_vals[losers]) if n_los>0 else 0
    med_win = np.median(feat_vals[winners]) if n_win>0 else 0

    # Tentar corte em varios percentis dos losers
    for pct in [25, 33, 50, 67, 75]:
        corte = np.percentile(feat_vals[losers], pct)

        # Filtro: remover entradas onde feature > corte (ou < corte)
        for op in ["maior", "menor"]:
            ...
            if len(pnls_filtrado) < MIN_TRADES_IS//2: continue

            w_f=pnls_filtrado[pnls_filtrado>0]
            l_f=pnls_filtrado[pnls_filtrado<=0]
            if len(l_f)==0 or len(w_f)==0: continue
...
if not melhores_filtros: return None
...
if session==“am”:  return ind[“session_am”].astype(bool)
elif session==“pm”: return ind[“session_pm”].astype(bool)
return np.ones(len(ind[“close”]),dtype=bool)

def h1(x): return np.roll(x,1)

def gerar_sinais(estrategia,ind,params):
d=params.get(“direction”,“short”); ses=params.get(“session”,“all”)
mask=mascara_sessao(ind,ses); c=ind[“close”]
ent=ext=None

```
if estrategia=="vwap_reversion":
    vwap=ind["vwap"]; std=ind["vwap_std"]; mult=params["vwap_std"]
    rsi=ind.get(f"rsi_{params['rsi_period']}")
    if rsi is None: return None,None
    lvl=params["rsi_level"]
    if d=="long":
        ent=(c<vwap-mult*std)&(rsi<lvl)&(h1(rsi)>=lvl); ext=c>vwap
    else:
        ent=(c>vwap+mult*std)&(rsi>(100-lvl))&(h1(rsi)<=(100-lvl)); ext=c<vwap

elif estrategia=="vwap_breakout":
    vwap=ind["vwap"]; vr=ind["vol_ratio"]; vc=params["vol_confirm"]
    if d=="long":
        ent=(c>vwap)&(h1(c)<=h1(vwap))&(vr>vc); ext=c<vwap
    else:
        ent=(c<vwap)&(h1(c)>=h1(vwap))&(vr>vc); ext=c>vwap

elif estrategia=="vwap_pullback":
    vwap=ind["vwap"]; ema=ind.get(f"ema_{params['ema_period']}")
    rsi=ind.get(f"rsi_{params['rsi_period']}"); rf=params["rsi_filter"]
    if ema is None or rsi is None: return None,None
    tol=ind["atr_14"]*0.3
    if d=="long":
        ent=(ema>vwap)&(np.abs(c-vwap)<tol)&(rsi>rf); ext=c<vwap
    else:
        ent=(ema<vwap)&(np.abs(c-vwap)<tol)&(rsi<(100-rf)); ext=c>vwap

elif estrategia=="orb_breakout":
    om=params["orb_minutes"]
    orh=ind.get(f"orb_high_{om}"); orl=ind.get(f"orb_low_{om}")
    if orh is None: return None,None
    vr=ind["vol_ratio"]; vc=params.get("vol_confirm",1.0)
    if d=="long":
        ent=(c>orh)&(h1(c)<=h1(orh))&(vr>vc); ext=c<orl
    else:
        ent=(c<orl)&(h1(c)>=h1(orl))&(vr>vc); ext=c>orh

elif estrategia=="orb_retest":
    om=params["orb_minutes"]
    orh=ind.get(f"orb_high_{om}"); orl=ind.get(f"orb_low_{om}")
    if orh is None: return None,None
    tol=ind["atr_14"]*0.5; above=c>orh
    if d=="long":
        foi=pd.Series(above).rolling(20).max().values.astype(bool)
        ent=foi&(np.abs(c-orh)<tol); ext=c<orl
    else:
        foi=pd.Series(~above).rolling(20).max().values.astype(bool)
        ent=foi&(np.abs(c-orl)<tol); ext=c>orh

elif estrategia=="rsi_vwap_combo":
    rsi=ind.get(f"rsi_{params['rsi_period']}"); vwap=ind["vwap"]
    side=params["vwap_side"]; lvl=params["rsi_level"]
    if rsi is None: return None,None
    vc=(c>vwap) if side=="above" else (c<vwap)
    if d=="long":
        ent=(rsi<lvl)&(h1(rsi)>=lvl)&vc; ext=rsi>50
    else:
        ent=(rsi>(100-lvl))&(h1(rsi)<=(100-lvl))&(~vc); ext=rsi<50

elif estrategia=="rsi_ema_vwap":
    rsi=ind.get(f"rsi_{params['rsi_period']}")
    ema=ind.get(f"ema_{params['ema_period']}"); vwap=ind["vwap"]
    lvl=params["rsi_level"]
    if rsi is None or ema is None: return None,None
    if d=="long":
        ent=(rsi<lvl)&(h1(rsi)>=lvl)&(c>ema)&(c>vwap); ext=rsi>55
    else:
        ent=(rsi>(100-lvl))&(h1(rsi)<=(100-lvl))&(c<ema)&(c<vwap); ext=rsi<45

elif estrategia=="rsi_vwap_session":
    rsi=ind.get(f"rsi_{params['rsi_period']}"); vwap=ind["vwap"]
    ovs=params["oversold"]; ovb=params["overbought"]; el=params["exit_level"]
    if rsi is None: return None,None
    if d=="long":
        ent=(rsi<ovs)&(h1(rsi)>=ovs)&(c>vwap); ext=rsi>el
    else:
        ent=(rsi>ovb)&(h1(rsi)<=ovb)&(c<vwap); ext=rsi<(100-el)

elif estrategia=="atr_channel_breakout":
    ep=params["ema_period"]; ap=params["atr_period"]; am=params["atr_mult"]
    key=f"kc_{ep}_{ap}_{str(am).replace('.','')}"
    ku=ind.get(f"{key}_upper"); kl=ind.get(f"{key}_lower")
    if ku is None: return None,None
    if d=="long":
    ...
    pp=params["momentum_period"]; pt=params["momentum_thresh"]
    roc=np.empty(len(c)); roc[:pp]=np.nan
    roc[pp:]=(c[pp:]-c[:-pp])/(c[:-pp]+1e-9)*100
    if d=="long": ent=roc>pt; ext=roc<0
    else: ent=roc<-pt; ext=roc>0

elif estrategia=="macd_vwap":
    mf=params["macd_fast"]; ms=params["macd_slow"]
    hist=ind.get(f"macd_{mf}_{ms}_hist"); vwap=ind["vwap"]
    if hist is None: return None,None
    if d=="long":
        ent=(hist>0)&(h1(hist)<=0)&(c>vwap); ext=(hist<0)&(h1(hist)>=0)
    else:
        ent=(hist<0)&(h1(hist)>=0)&(c<vwap); ext=(hist>0)&(h1(hist)<=0)

elif estrategia=="macd_rsi_vwap":
    cfg=params["macd_config"]; hist=ind.get(f"macd_{cfg}_hist")
    rsi=ind.get(f"rsi_{params['rsi_period']}"); rf=params["rsi_filter"]
    vwap=ind["vwap"]
    if hist is None or rsi is None: return None,None
    if d=="long":
        ent=(hist>0)&(h1(hist)<=0)&(rsi>rf)&(c>vwap); ext=(hist<0)&(h1(hist)>=0)
    else:
        ent=(hist<0)&(h1(hist)>=0)&(rsi<(100-rf))&(c<vwap); ext=(hist>0)&(h1(hist)<=0)

elif estrategia=="ema_vwap_trend":
    ef=ind.get(f"ema_{params['fast']}"); es=ind.get(f"ema_{params['slow']}")
    vwap=ind["vwap"]
    if ef is None or es is None: return None,None
    if d=="long":
        ent=(ef>es)&(h1(ef)<=h1(es))&(c>vwap); ext=ef<es
    else:
        ent=(ef<es)&(h1(ef)>=h1(es))&(c<vwap); ext=ef>es

elif estrategia=="dual_ema_momentum":
    ef=ind.get(f"ema_{params['fast']}"); es=ind.get(f"ema_{params['slow']}")
    vr=ind["vol_ratio"]; vc=params["vol_confirm"]
    if ef is None or es is None: return None,None
    if d=="long":
        ent=(ef>es)&(h1(ef)<=h1(es))&(vr>vc); ext=ef<es
    else:
        ent=(ef<es)&(h1(ef)>=h1(es))&(vr>vc); ext=ef>es

elif estrategia=="bb_squeeze_breakout":
    bp=params["bb_period"]; bs=params["bb_std"]
    sm=params["squeeze_mult"]; vc=params["vol_confirm"]
    key=f"bb_{bp}_{str(bs).replace('.','')}"
    bw=ind.get(f"{key}_width"); bu=ind.get(f"{key}_upper"); bl=ind.get(f"{key}_lower")
    vr=ind["vol_ratio"]
    if bw is None: return None,None
    bwa=pd.Series(bw).rolling(20).mean().values; squeeze=bw<bwa*sm
    ...
    bp=params["bb_period"]; bs=params["bb_std"]
    key=f"bb_{bp}_{str(bs).replace('.','')}"
    pct=ind.get(f"{key}_pct"); rsi=ind.get(f"rsi_{params['rsi_period']}")
    rc=params["rsi_confirm"]; vwap=ind["vwap"]
    if pct is None or rsi is None: return None,None
    if d=="long":
        ent=(pct<0.05)&(rsi<rc)&(c>vwap); ext=pct>0.5
    else:
        ent=(pct>0.95)&(rsi>(100-rc))&(c<vwap); ext=pct<0.5

elif estrategia=="donchian_vwap":
    dh=ind.get(f"don_high_{params['don_period']}")
    dl=ind.get(f"don_low_{params['don_period']}")
    vwap=ind["vwap"]; vr=ind["vol_ratio"]; vc=params["vol_confirm"]
    if dh is None: return None,None
    if d=="long":
    ...
    vwap=ind["vwap"]; ovs=params["oversold"]; ovb=params["overbought"]
    if k is None: return None,None
    if d=="long":
        ent=(k<ovs)&(h1(k)>=ovs)&(c>vwap); ext=k>50
    else:
        ent=(k>ovb)&(h1(k)<=ovb)&(c<vwap); ext=k<50

elif estrategia=="stoch_ema_vwap":
    k=ind.get(f"stoch_k_{params['stoch_period']}")
    ema=ind.get(f"ema_{params['ema_period']}"); vwap=ind["vwap"]
    ovs=params["oversold"]; ovb=params["overbought"]
    if k is None or ema is None: return None,None
    if d=="long":
        ent=(k<ovs)&(h1(k)>=ovs)&(c>ema)&(c>vwap); ext=k>50
    else:
        ent=(k>ovb)&(h1(k)<=ovb)&(c<ema)&(c<vwap); ext=k<50

elif estrategia=="volume_spike_reversal":
    vz=ind.get("vol_z_20"); rsi=ind.get(f"rsi_{params['rsi_period']}")
    vs=params["vol_spike"]; lvl=params["rsi_level"]
    if vz is None or rsi is None: return None,None
    spike=vz>vs
    if d=="long":
        ent=spike&(rsi<lvl)&(h1(rsi)>=lvl); ext=rsi>55
    else:
        ent=spike&(rsi>(100-lvl))&(h1(rsi)<=(100-lvl)); ext=rsi<45

elif estrategia=="volume_vwap_momentum":
    vr=ind["vol_ratio"]; vwap=ind["vwap"]; vc=params["vol_confirm"]
    if d=="long":
        ent=(c>vwap)&(h1(c)<=h1(vwap))&(vr>vc); ext=c<vwap
    else:
        ent=(c<vwap)&(h1(c)>=h1(vwap))&(vr>vc); ext=c>vwap
    ...
    vwap=ind["vwap"]; thr=params["cci_thresh"]
    if cci is None: return None,None
    if d=="long":
        ent=(cci<-thr)&(h1(cci)>=-thr)&(c>vwap); ext=cci>0
    else:
        ent=(cci>thr)&(h1(cci)<=thr)&(c<vwap); ext=cci<0
    ...
    if rsi is None: return None,None
    if d=="long":
        ent=(rsi<ovs)&(h1(rsi)>=ovs); ext=rsi>el
    else:
        ent=(rsi>ovb)&(h1(rsi)<=ovb); ext=rsi<(100-el)
    ...
    if rsi is None: return None,None
    if d=="long":
        ent=(c>orh)&(h1(c)<=h1(orh))&(vr>vc)&(rsi>rf)&(c>vwap); ext=c<orl
    else:
        ent=(c<orl)&(h1(c)>=h1(orl))&(vr>vc)&(rsi<(100-rf))&(c<vwap); ext=c>orh
else:
    return None,None

if ent is None: return None,None
ent=ent&mask; ent[0]=False
...
GRIDS = {
# === VWAP — maior familia, 750k combos ===
“vwap_reversion”: {
...
ind = ind_is  # ja e dict de numpy arrays
keys   = list(grid.keys())
combos = list(itertools.product(*[grid[k] for k in keys]))
if mini: combos = combos[:20]

```
validos=[]; atr_med=float(np.nanmean(ind["atr_14"])); t0=time.time()

for combo in combos:
    params=dict(zip(keys,combo))
    try:
        ent,ext=gerar_sinais(estrategia,ind,params)
        if ent is None or ent.sum()<10: continue
        sl=atr_med*params.get("atr_sl",1.0); tp=sl*params.get("rr",2.0)
...
return {
    "estrategia":estrategia,"n_combos":len(combos),
    "n_validos":len(validos),"elapsed":round(time.time()-t0,1),
    "top10":validos[:10],
}
```
...
while i+janela<=len(datas):
    d_s=datas[i]; d_e=datas[min(i+janela-1,len(datas)-1)]
    mask_j=(df_oos.index.normalize()>=d_s)&(df_oos.index.normalize()<=d_e)
    if mask_j.sum()<100:
        i+=janela; continue

    # Sub-ind da janela
    ind_j={k:v[mask_j] if isinstance(v,np.ndarray) and len(v)==len(df_oos)
           else v for k,v in ind_oos_full.items()}
    ...
    sl=atr_j*params.get("atr_sl",1.0); tp=sl*params.get("rr",2.0)
...
return {
    "janelas":len(resultados),"lucrativas":luc,
    "wfe_pct":round(wfe,1),
...
b=np.zeros(200,dtype=np.bool_); b[10]=True
_=simular_long(d,d,d,b,b,20.,40.,MULT,COMM,SLIP)
_=simular_short(d,d,d,b,b,20.,40.,MULT,COMM,SLIP)
_=simular_com_contexto(d,d,d,b,b,20.,40.,MULT,COMM,SLIP,d,d,d,d,d,d)
_=calcular_mae_mfe(d,d,d,b,1)
print("[JIT] Pronto!", flush=True)

# Rodar sequencial com Numba (evita problema de memoria do multiprocessing)
print(f"\n[GRID] {len(GRIDS)} estrategias sequencial + Numba...", flush=True)
t0_total=time.time(); todos=[]

for estrategia, grid in GRIDS.items():
    res = worker_estrategia((estrategia, grid, ind_is, MINI))
    e=res["estrategia"]; spd=res["n_combos"]/max(res["elapsed"],0.1)
    top=res["top10"][0] if res["top10"] else None
    pf_s=f"PF={top['pf']:.3f}" if top else "sem validos"
    print(f"  [{e.upper():25}] {res['n_validos']:>7,}/{res['n_combos']:>9,} "
          f"| {res['elapsed']:.1f}s | {spd:.0f}/s | {pf_s}", flush=True)
    if res["top10"]:
...
for r in todos[:20]:
    print(f"  {r['estrategia']:25} {r['pf']:>6.3f} {r['wr']:>5.1f} "
          f"{r['n']:>6} {r['exp']:>7.2f}", flush=True)

# MAE/MFE + Analise de Falhas nos top 20
print(f"\n{'='*68}", flush=True)
print(f"  MAE/MFE + ANALISE DE FALHAS", flush=True)
atr_med_is=float(np.nanmean(ind_is["atr_14"]))
candidatos=[]

for r in todos[:20]:
    estrategia=r["estrategia"]; params=r["params"]
    try:
        ent,ext=gerar_sinais(estrategia,ind_is,params)
        if ent is None or ent.sum()<20: continue

        # MAE/MFE
        d=1 if params.get("direction","short")=="long" else -1
        ...
        if filtro and filtro["pf_depois"]>pf_base:
            ...
            pnls_f=executar(ind_is,ent_filtrada,ext,sl_opt,tp_opt,
                            params.get("direction","short"))
            m_f=metricas(pnls_f,min_trades=MIN_TRADES_IS//2)
            if m_f: pf_com_filtro=m_f["pf"]

        print(f"  {estrategia:25} "
              f"RR_real={rr_real:.2f} "
              f"PF_base={pf_base:.3f} "
              f"PF_filtro={pf_com_filtro:.3f}", end="", flush=True)

        if filtro:
            print(f" [FILTRO: {filtro['feature']}"
                  f" {filtro['operacao']} {filtro['corte']:.1f}"
                  f" +{filtro['melhoria']:.0f}%]", flush=True)
        else:
            print(flush=True)
...
# OOS Rolling nos candidatos promissores
print(f"\n{'='*68}", flush=True)
...
for cand in candidatos_oos:
    estrategia=cand["estrategia"]; params=cand["params"]
    sl=cand["sl_opt"]; tp=cand["tp_opt"]

    print(f"\n  {estrategia} PF={cand['pf_filtro']:.3f}", flush=True)
    oos_r=oos_rolling(estrategia,df_oos,params,ind_oos)

    is_pf=cand["pf_filtro"]; oos_pf=oos_r["pf_medio"] if oos_r else 0
    ...
    print(f"  IS={is_pf:.3f} OOS={oos_pf:.3f} "
          f"Deg={deg:.1f}% WFE={wfe:.0f}% "
          f"{'✅ APROVADO' if ok else '❌'}", flush=True)

    if oos_r:
        for j in oos_r["detalhes"]:
            print(f"    {j['data']} PF={j['pf']:.3f} "
                  f"T={j['trades']} {'✅' if j['lucrativo'] else '❌'}", flush=True)
...
print(f"  {'ESTRATEGIA':25} {'IS':>6} {'OOS':>6} {'DEG':>6} {'WFE':>5} {'FILTRO':>20} {'STATUS':>10}", flush=True)
...
for r in sorted(resultados_finais,key=lambda x:-x["pf_is"]):
    ...
    print(f"  {r['estrategia']:25} "
          ...
          f"{fl_str:>20} "
          f"{'✅' if r['aprovado'] else '❌':>10}", flush=True)
